...
export class EntitiesService {
  ...
  getEntities() {
    return this.http.get(environment.applicationUrl + 'entities/').toPromise();
  }

  getEntity(id) {
    return this.http.get(environment.applicationUrl + `entities/${id}`).toPromise();
  }
  ...
  create_entity(entity) {
    return this.http.post(environment.applicationUrl + `entities/`, entity).toPromise();
  }

  update_entity(entity) {
    return this.http.put(environment.applicationUrl + `entities/${entity._id.$oid}`, entity).toPromise();
  }

  delete_entity(id) {
    return this.http.delete(environment.applicationUrl + `entities/${id}`, {}).toPromise();
  }
}
